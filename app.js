// Professional Question Paper Generator - COMPLETE VERSION WITH 16 MARKS
class ProfessionalQuestionPaperGenerator {
    ...
    bindEvents() {
        ...
        // Enhanced auto-detect learning level with real-time feedback
    ...
    enhancedAutoDetectLearningLevel() {
        ...
        // Enhanced detection with scoring system
        let scores = { 'L1': 0, 'L2': 0, 'L3': 0, 'L4': 0, 'L5': 0, 'L6': 0 };
        let detectedLevel = 'L2'; // Default
        let confidence = 0;

        // Check for keywords in each learning level with weighted scoring
        ...
        for (const [level, score] of Object.entries(scores)) {
            if (score > maxScore) {
                maxScore = score;
                detectedLevel = level;
                confidence = Math.min(Math.round((score / 3) * 100), 100);
            }
        }

        // Special patterns for more accurate detection
        if (questionText.includes('define') && questionText.includes('with example')) {
            detectedLevel = 'L2';
            confidence = 90;
        }
        
        if (questionText.includes('calculate') || questionText.includes('find the')) {
            detectedLevel = 'L3';
            confidence = 95;
        }

        if (questionText.includes('design') || questionText.includes('create') || questionText.includes('develop')) {
            detectedLevel = 'L6';
            confidence = 95;
        }

        // Update the UI with detected level
        const levelNames = {
            'L1': 'Remember',
            'L2': 'Understand', 
            'L3': 'Apply',
        ...
        // Store the detected level for submission
    ...
    handleQuestionSubmit() {
        ...
        // Use AI-detected learning level
        const learningLevel = this.detectedLearningLevel || 'L2';
        ...
        if (this.currentEditId) {
            ...
            questionLines.forEach(line => {
                ...
                if (cleanQuestion && cleanQuestion.length > 0) {
                    // Detect learning level for each individual question
            ...
            // Add each question with its detected level
            ...
            this.updateUI();
            this.saveData();
            this.showSuccessMessage(`${validQuestions.length} question(s) added with AI detection!`);
        }
    }
    ...
    detectLearningLevelForText(text) {
        ...
        // Check for keywords with weighted scoring
        for (const [level, keywordGroups] of Object.entries(this.learningKeywords)) {
            for (const keyword of keywordGroups.primary) {
    ...
    editQuestion(id) {
        ...
        // Update the detected level display
        const detectedLevelSpan = document.getElementById('detectedLevel');
        const levelNames = {
    ...
    clearForm() {
        ...
        // Reset the AI detection display
        const detectedLevelSpan = document.getElementById('detectedLevel');
        detectedLevelSpan.innerHTML = 'AI will auto-detect';
        detectedLevelSpan.className = 'detected-level';
    ...
    generatePaper() {
        ...
        const num14mark = parseInt(document.getElementById('num14mark').value) || 0;
        ...
        if (num2mark + num4mark + num8mark + num13mark + num14mark + num15mark + num16mark === 0) {
        ...
        const requirements = [
            ...
            {marks: 14, needed: num14mark, available: counts[14] || 0},
            {marks: 15, needed: num15mark, available: counts[15] || 0},
            {marks: 16, needed: num16mark, available: counts[16] || 0}
        ];
        ...
        const paper = this.createProfessionalQuestionPaper({
            num2mark, num4mark, num8mark, num13mark, num14mark, num15mark, num16mark,
    ...
    createProfessionalQuestionPaper(config) {
        ...
        // Generate sections for all mark categories
        ...
        if (config.num14mark > 0) {
            paper.sections['14'] = this.selectQuestionsBalanced(14, config.num14mark, config.includeOrQuestions);
        }
        if (config.num15mark > 0) {
            paper.sections['15'] = this.selectQuestionsBalanced(15, config.num15mark, config.includeOrQuestions);
        }
        if (config.num16mark > 0) {
            paper.sections['16'] = this.selectQuestionsBalanced(16, config.num16mark, config.includeOrQuestions);
        }
    ...
    displayGeneratedPaper(paper) {
        ...
        // Part B - Higher marks (4, 8, 13, 14, 15, 16)
        ...
        this.showSuccessMessage('Professional question paper generated with AI-detected learning levels!');
    }
